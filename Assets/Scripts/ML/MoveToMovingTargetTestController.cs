...
public class MoveToMovingTargetTestController : MonoBehaviour
{
    ...
    private PlayerAgent_ShootMovingTarget playerAgentShooting;
    ...
    void Start()
    {
        ...
        playerAgentShooting = agentTank.GetComponent<PlayerAgent_ShootMovingTarget>();
    ...
    void Update()
    {
        ...
        else if (playerAgentShooting != null)
        {
            if (currentTimesReachedTarget != playerAgentShooting.timesReachedObjective)
            {
                currentTimesReachedTarget = playerAgentShooting.timesReachedObjective;
                targetController.ResetSeededRNG(currentTimesReachedTarget);
                targetController.GetNewPositionAndDestination();
                agentTank.transform.position = agentInitialPosition;
                agentTank.transform.rotation = agentInitialRotation;
            ...
            if (playerAgentShooting.timesReachedObjective == timesToReachTarget)
            {
                Debug.Log("Elapsed time: " + elapsedTime + "\nModel Name: " + behaviorParameters.Model.name);
                EditorApplication.ExitPlaymode();
            }
